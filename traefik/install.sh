#!/bin/bash
#
# This script installs and updates Traefik on a Docker Stack.
# Traefik is used to provide various services on a server.
#
# You can use these parameters to install and configure Traefik:
#
...
###########################################################
# Create the network for Traefik Proxy (traefik-proxy).
###########################################################
function create_network() {
  docker network inspect traefik-proxy --format {{.ID}} 2>/dev/null | grep -v -e "^$" || docker network create -d overlay traefik-proxy
}
...
###########################################################
# Check whether Docker is the swarm manager.
###########################################################
function is_docker_swarm_manager() {
  [[ "$(docker info 2>/dev/null | grep 'Is Manager'| awk -F ' ' '{print $3}')" == "true" ]]
}
...
# check whether Docker is swarm manager.
if ! is_docker_swarm_manager; then
  echo "Docker is not a swarm manager."
  exit 22
fi

# create the network if the network doesn't exists.
...
if is_deployed "$name"; then
  printf "\n"
  printf "Docker Stack for Traefik is already deployed.\n"
  printf "Docker Stack for Traefik will be updated.\n"
  printf "\n"
else
  printf "\n"
  printf "Docker Stack for Traefik is not deployed.\n"
  printf "Docker Stack for Traefik will be deployed.\n"
  printf "\n"
fi
...
# deploy the Docker Stack for Traefik.
docker stack deploy -c "$folder/docker-compose.yml" "$name" 1> /dev/null

# check whether the Docker Stack was deployed successfully.
if ! is_deployed "$name"; then
  printf "\nDocker Stack for Traefik could not be deployed.\n\n"
  exit 32
else
  printf "\nDocker Stack for Traefik was deployed successfully.\n\n"
  exit 0
fi
